...
the minimal mean square error (MSE) $1-\frac{k}{n}$ on average,
if the value domains of $g, f_i$ has only $n$ elements from $\mathbb{R}$,
which are i.i.d. sampled from the Gaussian distribution.
However, if we introduce a non-linear function $\sigma$ and use
$\sigma(\sum_{i=1}^k w_i g_i)$ to approximate $f$,
we can achieve
MSE lower than $\frac{k}{n}$. This paper \textbf{quantitatively} investigates
the MSE 
that the non-linear
...
candidate to achieve the minimal MSE in the problem mentioned above.
...
Suppose the value domains of $f_i, g$ are with cardinality $n$, 
then the functions themselves can be completely determined by $n$-dimensional vectors.
Furthermore, we assume each element from the value domains is sampled from a distribution,
since we are considering the average performance over different $f, g_i$ pairs.
Then we can establish a correspondence between
$g_i$ and $\underline{x}_i$,
and between $f$ and $\underline{y}$.
