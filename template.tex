...
Consider a function approximation problem in which we use $\underline{X}_1, \dots, \underline{X}_k$
to approximate a target vector $\underline{Y}$.
When we only use linear combinations $\sum_{i=1}^k w_i \underline{X}_i$
to approximate $\underline{Y}$, we can only achieve
the minimal mean square error (MSE) $\frac{k}{n}$,
if $\underline{Y}, \underline{X}_i$ are i.i.d. Gaussian vectors.
However, if we introduce a non-linear function $\sigma$ and use
$\sigma(\sum_{i=1}^k w_i \underline{X}_i)$ to approximate $\underline{Y}$,
we can achieve
MSE lower than $\frac{k}{n}$. This paper investigates \textbf{quantitatively} the MSE 
that non-linear
...
It is well-known that with only one hidden layer and choosing sigmoid as activation function,
we can use neural network to approximate any continuous function\cite{cybenko1989approximation}.
However, this universal approximation theorem only requires the non-linear activation satisfies certain
regularity conditions and does not tell us which kind of activation function works best
in a certain problem. On the contrary, this paper shows that the Hermite polynomial is the optimal
candidate to achieve the smallest MSE in the problem mentioned above.

While other works empirically choose better activation functions \cite{ramachandran2017searching},
...
achieve
the minimal MSE (or neural network loss).
...
The network we considered can be regarded as a special ResNet model,
which is widely used in deep neural network architecture.
% This network can be regarded as a kind of non-linear regression.
We use an averaged MSE to measure the network loss,
from which we can obtain universally optimal activation function for the given network,
and it's found that the averaged loss is minimized when the activation function is Hermite orthogonal polynomial, which
