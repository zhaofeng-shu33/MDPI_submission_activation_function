...
Consider a functional approximation problem in which we use $X_1, \dots, X_k$
to approximate a target vector $Y$.
When we only use linear combinations $\sum_{i=1}^k w_i X_i$
to approximate $Y$, we can only achieve
the mean square error (MSE) $\E[||Y-\sum_{i=1}^k X_i||^2]=\frac{k}{n}$,
if $Y, X_i$ are i.i.d. Gaussian vectors.
However, if we introduce a non-linear function $\sigma$ and use
$\sigma(\sum_{i=1}^k w_i X_i)$ to approximate $Y$, we can achieve
lower MSE. This paper investigates \textbf{quantitatevely} the MSE non-linear
approximator achives, compared with its linear counterpart.

The functional approximation problem revealed above can also be formulated in 
...
However, this universal approximation theorem only requies the non-linear activation satisfies certain
regularity conditions and does not tell us which kind of activation function works best
in a certain problem. This paper shows that the Hermite polynomial is the optimal
candidate to achieve smallest MSE.

While other works empirically choose better activation functions \cite{ramachandran2017searching},
to our best knowledge our work firstly quantify the performance gain of functional approxmiation.
