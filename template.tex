...
\secondnote{\deleted{These authors contributed equally to this work.}}
...
we can achieve
an averaged RSS lower than $(1-\frac{k}{n})$. This paper \replaced{quantitatively}{\textbf{quantitatively}} investigates
the RSS
that the non-linear
...
On the other hand, our formulation not only illustrates the role nonlinearity plays in activation function,
...
we use the one-node neural network which outputs one-dimensional feature \cite{dey2018approximation}.
...
the best performance that non-linear activation function can achieve for a certain network architecture.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{\replaced{Models and methods}{Mathematical Model}}\label{sec:mm}
...
\added{The one-node neural network is the output layer in a complex neural network for regression task, where
...
To make fair comparison between different
non-linear term $\xi$ for given $\epsilon$, we apply the normalization constraint
...
\added{For assumption 3), notice that when we project a random vector $\underline{Y}$ into a fixed linear subspace,
...
Proposition \ref{prop:linear} says that the averaged \replaced{residue error}{MMSE $\mathrm{E}$} for linear function is equal to $(1-\frac{k}{n})$,
...
\section{Hermite \replaced{p}{P}olynomials}\label{sec:hp}
...
\section{\replaced{Experimental validation and discussions}{Experiment Result}}\label{sec:er}
...
\added{Currently this work only investigates one-node neural network,} \replaced{h}{H}ow non-linearity works in \deleted{more complex} neural networks \added{with more neurons} will be considered in the future. 
...
Shao-Lun Huang. \added{These authors contributed equally to this work.}
...
as the quadratic form $ \underline{q}^T \bm{M} \underline{q} $\replaced{,}{.}
