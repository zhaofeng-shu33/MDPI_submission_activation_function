...
\Author{\hl{Feng Zhao} $^{1}$\orcidA{} and Shao-Lun Huang $^{2,}$*}
...
\address{%
$^{1}$ \quad Department of Electronics Engineering, Tsinghua University, \hl{Beijing,} %MDPI: Please add the postal code (or ZIP code in the U.S.). If the postal code is not available, please provide the P.O. Box.
 China; zhaof17@mails.tsinghua.edu.cn\\
$^{2}$ \quad Tsinghua-Berkeley Shenzhen \hl{Institute,} %MDPI: Please check that the address information is complete. The provided information should be arranged from subordinate to superior.
 \hl{Shenzhen,} China}
...
we use $X,\underline{X}$ and \highlighting{$\bm{X} $} %MDPI: Is the bold necessary?
...
\caption{\hl{The} structure of a one-node neural network. The activation function $\sigma$ takes the form $\sigma(z)=z + \epsilon \xi(z)$.}%MDPI: Figure should be after where it is first mentioned in the text. So we move it here, please confirm.
\label{fig:ns}
\end{figure}
...
\newblock An Approximation Algorithm for training One-Node ReLU Neural Network.  \hl{\emph{arXiv} \textbf{2018},	arXiv:1810.03592.}%MDPI: Newly added  information. Please confirm.
...
\newblock In Proceedings of the 6th International Conference on Learning
  Representations, {ICLR} 2018, Vancouver, BC, Canada, 30 April--3 May \hl{2018.} %MDPI: Deleted the ``'Conference Track Proceedings,  2018.', please confirm
...
\newblock In Proceedings of the 2016 IEEE Conference on Computer Vision and
  Pattern Recognition (CVPR),  \hl{Las Vegas, Nevada, USA, 27--30 June 2016;} pp. 770--778.
\newblock {{https://doi.org/10.1109/CVPR.2016.90}}.%newly added information, please confirm
...
\newblock  Available online: \url{http://dlmf.nist.gov/}  \hl{(accessed on).} %MDPI: Please add day month year, and it should be before the submission date (19 September 2022)
